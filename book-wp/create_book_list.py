...
ratings = pd.read_csv(path_original_ratings, usecols=['isbn', 'Summary', 'Category'], dtype={'isbn': str})
ratings = ratings.drop_duplicates()
ratings['Category'] = ratings['Category'].apply(lambda x: x.split('\'')[1])


average_ratings = pd.read_csv(path_original_ratings, usecols=['isbn', 'rating' ], dtype={'isbn': str})
...
merged.to_csv("C:\\Users\pauli\Work\Book Recommendation System\\book-wp\\books_complete.csv")
